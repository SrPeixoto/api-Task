...
if (isset($data['taskId'], $data['completed']) && is_bool($data['completed'])) {
    ...
    try {
        ...
        //     $dueDateTime = $Edit_taskDueDate . 'T' . $Edit_taskDueTime . ':00.000Z';
        //     $task->setDue($Edit_dueDateTime);
        // } elseif (!empty($Edit_taskDueDate)) {
        //     $task->setDue($Edit_taskDueDate . 'T00:00:00.000Z');
        // }

        $task->setTitle($data['Edit_title']);
        $task->setNotes($data['Edit_notes']);

        // $task->setDue($data['Edit_due_date']);
        $editdueDateTime = $data['Edit_due_date'] . 'T' . $Edit_taskDueTime . ':00.000Z';
        $task->setDue($editdueDateTime);

        //$task->setDue($data['Edit_due_time']);
        $task->setDue($data['Edit_due_time'] . 'T00:00:00.000Z');
